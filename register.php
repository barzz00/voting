...
if (isset($_POST["register"])) {
    ...
    $check_query->bind_param("s", $schoolID);
    $check_query->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        // Debugging: check if OTP is generated
        echo "Generated OTP: " . $otp; // You can remove this later
        ...
        $insert_query = $conn->prepare("INSERT INTO login (schoolID, full_name, phone, course, year, email, password, otp, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $insert_query->bind_param("ssssssssi", $schoolID, $full_name, $phone, $course, $year, $email, $password_hash, $otp, $status);
        ...
        if ($insert_query->execute()) {
            ...
            // Print the error message to see why the query failed
